...
Infrastructura de testare este importantă deoarece permite verificarea aplicației la mai multe niveluri fără a dubla artificial codul de test. Figura~\ref{fig:niveluri-testare} ilustrează piramida de testare adoptată în proiect.
...
\begin{tikzpicture}[
	...
	layer/.style={draw, rounded corners, align=center, minimum width=8.5cm, minimum height=0.9cm},
	arrow/.style={-Latex, thick}
]
\node[layer, fill=gray!15] (l1) {Validarea DTO-urilor și a regulilor simple};
\node[layer, fill=gray!10, below=of l1] (l2) {Teste pentru servicii și logică de business};
\node[layer, fill=gray!15, below=of l2] (l3) {Teste pentru controllere și contractele API};
\node[layer, fill=gray!10, below=of l3] (l4) {Teste de integrare pentru fluxuri complete};
\node[layer, fill=gray!15, below=of l4] (l5) {Validări operaționale și verificări manuale};
...
\draw[arrow] (l4) -- (l5);
\end{tikzpicture}
\caption{Nivelurile principale de testare folosite în evaluarea proiectului}
\label{fig:niveluri-testare}
\end{figure}
