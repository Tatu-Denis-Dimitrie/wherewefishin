...
În ASP.NET Core \citep{aspnetcore}, validarea JWT este configurată explicit prin parametrii de validare injectați la înregistrarea middleware-ului: issuer-ul, audience-ul, durata de viață și cheia de semnare sunt verificate la fiecare cerere. Fereastra de toleranță pentru expirare (\texttt{ClockSkew}) este setată la zero, ceea ce înseamnă că un token expirat este respins imediat, fără perioadă de grație. Odată validat, tokenul este decodat, iar claims-urile sunt populate în \texttt{HttpContext.User}, disponibile în orice punct al pipeline-ului.
...
Serviciul \texttt{frontend} compilează aplicația Angular în fișiere statice și le servește prin nginx, care acționează și ca reverse proxy pentru cererile API, eliminând problemele CORS în producție. Serviciul \texttt{api} rulează backend-ul .NET și depinde de serviciul de bază de date: Docker Compose garantează că baza de date este pregătită înainte de pornirea API-ului, prin configurarea ordinii de dependențe și a health check-urilor. Serviciul \texttt{ai-service} rulează independent și expune un singur endpoint de inferență pe rețeaua internă. Serviciul \texttt{database} folosește imaginea oficială SQL Server pe Linux.
