...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/harta.png}
\caption{Harta interactivă cu locațiile de pescuit marcate pe teritoriul României}
\label{fig:harta-principala}
\end{figure}
...
\textbf{Decizii de proiectare} (alegeri argumentate, nu rezultate științifice):
\begin{enumerate}
  \item integrarea într-o singură platformă a funcțiilor de descoperire, rezervare, administrare și analiză media, alegere care reflectă o viziune unitară asupra fluxurilor operaționale ale pescuitului recreativ.
  \item modelul de roluri cu patru niveluri (utilizator, angajat, manager, administrator), fiecare cu permisiuni bine definite și separare clară a responsabilităților.
  \item harta interactivă ca element central al navigării, inclusiv pentru delimitarea vizuală a pontoanelor prin geometrie poligonală.
  \item arhitectura modulară cu microserviciu separat pentru analiza media, potrivită pentru întreținere, testare și extindere ulterioară.
\end{enumerate}

\textbf{Contribuții aplicative} (artefacte produse efectiv în cadrul proiectului):
\begin{enumerate}
  \item construirea și adnotarea manuală a unui set de date propriu cu specii de pești din pescuitul recreativ local, absent din seturile de date publice generice (COCO, Pascal VOC).
  \item antrenarea și calibrarea unui model YOLOv8 specializat pe acest set de date, cu transfer learning de la greutăți preantrenate pe COCO.
  \item integrarea pipeline-ului de detecție YOLO cu mecanismul de tracking ByteTrack într-un microserviciu Python expus printr-un API HTTP consumabil de orice client.
  \item mecanismul de verificare prin cod QR și token unic pentru confirmarea prezenței la locație, mediat de angajat printr-o interfață optimizată pentru utilizare în teren.
  \item infrastructura de deployment containerizată, cu rate limiting diferențiat pe endpoint, expusă public prin Cloudflare Tunnel fără adresă IP statică.
\end{enumerate}
